	...
	if (count > 0) {
		...
		for (int i = 0; i < groupLedCount; i++) {
			...
			if (pos < valley) {
				...
				distanceWave = valley - 0.5f - pos;
			} else {
				color = flag ? group.color2 : group.color1;
				distanceWave = valley + 0.5f - pos;
			}
			if (distanceWave < 0.0f) {
				distanceWave = -distanceWave;
			}

			uint8_t scale;
			if (distanceWave >= 0.25f) {
				scale = 0;
			} else {
				float val = (distanceWave * 4.0f) * 255.0f;
				scale = 255 - ease8InOutApprox(val > 255.0f ? 255 : (uint8_t)val);
			}
	...
	if (count > 0) {
		...
		for (int i = 0; i < groupLedCount; i++) {
			...
			if (i >= gradientStartPosition - 1 && i < gradientStartPosition) {
				color = group.color1;
				float scaleVal = (i - (gradientStartPosition - 1)) * 255.0f;
				color.nscale8(scaleVal > 255.0f ? 255 : (uint8_t)scaleVal);
			} else if (i >= gradientStartPosition && i < gradientEndPosition) {
				float blendVal = ((i - gradientStartPosition) / gradientLength) * 255.0f;
				color = blend(group.color1, group.color2, blendVal > 255.0f ? 255 : (uint8_t)blendVal);
			} else if (i >= gradientEndPosition && i < gradientEndPosition + 1) {
				color = group.color2;
				float scaleVal = (1.0f - (i - gradientEndPosition)) * 255.0f;
				color.nscale8(scaleVal > 255.0f ? 255 : (uint8_t)scaleVal);
			}
